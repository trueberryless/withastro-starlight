...
Routendaten enthalten zum Beispiel eine Eigenschaft `siteTitle` mit einem Wert, der auf der Option [`title`](/de/reference/configuration/#title-erforderlich) in deiner Starlight-Konfiguration basiert, und eine Eigenschaft `toc`, die aus Überschriften im Inhalt der aktuellen Seite generiert wird.
