...
Starlightは、Astroの[`markdown.processor`](https://docs.astro.build/ja/reference/configuration-reference/#markdownprocessor)設定を使用してMarkdownとMDXをレンダリングします。設定したプロセッサーにプラグインやオプションを渡すことで、カスタムの構文や動作をサポートできます。詳しくは、Astroドキュメントの[「Markdownプラグイン」](https://docs.astro.build/ja/guides/markdown-content/#markdownプラグイン)を参照してください。
