...
Вы можете указать пользовательскую иконку для вставки в фигурных скобках после типа панели или [пользовательского заголовка](#настраиваемые-заголовки-вставок), например, `:::tip{icon="heart"}` или `:::tip[А вы знали?]{icon="heart"}` соответственно.
...
Starlight обрабатывает Markdown и MDX с помощью настраиваемого [`markdown.processor`](https://docs.astro.build/ru/reference/configuration-reference/#markdownprocessor) из Astro. Вы можете добавить поддержку собственного синтаксиса и поведения, передав плагины и параметры в используемый процессор. Подробнее см. в разделе [«Markdown Plugins»](https://docs.astro.build/ru/guides/markdown-content/#markdown-plugins) документации Astro.
