...
你可以在旁白类型或 [自定义标题](#自定义旁白标题) 后面的大括号中指定旁白的自定义图标，例如分别为 `:::tip{icon="heart"}` 或 `:::tip[你知道吗？]{icon="heart"}`。
图标名称必须是 [Starlight 内置图标之一](/zh-cn/reference/icons/#所有图标) 的名称。
...
Caution 和 danger 旁白有助于吸引用户注意可能绊倒他们的细节。
如果你发现自己经常使用这些，这也可能表明你正在记录的内容可以从重新设计中受益。
...
详细信息（也称为“手风琴（accordion）”）用于隐藏不立即相关的内容。
用户可以点击一个简短的摘要来展开并查看完整内容。
...
[脚注](https://www.markdownguide.org/extended-syntax/#footnotes) 可用于添加简短的参考或说明，而不会打断内容的流畅性。它们以带编号的上标链接的形式出现，点击后会跳转到页面底部收集的引用文本。

使用带有插入符号和编号引用的方括号来定义脚注：

```md
这里有一个脚注[^1]，后面还有一些其他文字。
...
Starlight 使用 Astro 的可配置 [`markdown.processor`](https://docs.astro.build/zh-cn/reference/configuration-reference/#markdownprocessor) 渲染 Markdown 和 MDX。 你可以通过向配置的处理器传递插件和选项来添加对自定义语法和行为的支持。请参阅 Astro 文档中的 [“Markdown 插件”](https://docs.astro.build/zh-cn/guides/markdown-content/#markdown-插件) 以了解更多信息。
